...
for n in range(start_step + 1, Nposes):
    ...
    if beacon_visible[n]:
        ...
        if sum(weights) < 1e-10:
            ...
            for m in range(Nparticles):
                poses[m] = (uniform(est_poses[n-1, 0] + Xmin*2, est_poses[n-1, 0] + Xmax*2),
                            uniform(est_poses[n-1, 1] + Ymin *2 , est_poses[n-1, 1] + Ymax),
                            uniform(est_poses[n-1, 2] + Tmin*2, est_poses[n-1, 2] + Tmax*2))
                weights[m] = 1
